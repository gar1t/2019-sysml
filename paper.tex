...
\begin{abstract}
  ...
  applications, however, present unique requirements that call for
  tool specialization. Guild AI combines the proven effectiveness of
...
software, rather than a producer or developer. A user may be a
software developer, in the case of consuming software libraries or
frameworks, or she may be an end-user who uses software as a program
with a graphical or command line interface. In both cases, packaged
software may be provided in units that the user can install and begin
using quickly and reliably.
...
\item Additional Python modules required by \verb|tensorflow| should
  also be installed.
\item Installed software will work as expected on the target system.
...
users are free to install and evaluate packages with the confidence
that they can safely remove that software later.
...
context of machine learning, models may be represented by source code,
either explicitly as declared structures, or implicitly by way of
executable instructions that perform model related tasks. A
...
that was generated using the \verb|guild package| command. They
contain Guild AI specific metadata that advertises models they
contain.
...
important role in Guild AI's ability to effectively use
...
operation is run, the run directory is used as the process working
directory. Operations should write run artifacts within the working
directory to ensure they are associated with the run.
...
directory is prepared with required sources, Guild AI starts the run
...
environments created using {\emph{virtualenv} \cite{venv}.
...
specified---to a different file system directory. This can be used to
...
When running Guild AI commands in an activated environment, the
following are environment specific:

\begin{itemize}
\item Installed packages and models
\item Runs
\end{itemize}
...
command. Runs may be deleted by status, operation, or label.
...
Guild AI is freely available under the Apache 2.0 \cite{guild} open source
...
reusable models \cite{pkg} that fulfill the goals outlined in this
paper:
